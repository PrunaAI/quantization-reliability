...
BEAM_SEARCH_STRATEGIES = [
    GenerationStrategy.BEAM_SEARCH,
    GenerationStrategy.BEAM_SEARCH_WITH_SAMPLING,
    GenerationStrategy.DIVERSE_BEAM_SEARCH
]

TOKEN_SCORES_ID_FIELDS = [
    'token_info_tuples',
    'full_decoded_text',
    'relevant_decoded_text',
    'effective_lengths',
    'padded_sequences',
    'semantic_entropy_scores',
    'log_likelihood_raw_scores'
]

# Ordered so more specific patterns (e.g. "llama32_") are checked before any
# accidental prefix collision — see model_name_to_model_family().
MODEL_NAME_FAMILY_PATTERNS = [
    ("gpt-2", "gpt-2"),
    ("gpt-3", "gpt-3"),
    ("bloomz", "bloomz"),
    ("tinyllama_", "tinyllama"),
    ("llama2_", "llama2"),
    ("llama3_", "llama3"),
    ("llama31_", "llama31"),
    ("llama32_", "llama32"),
...
def model_name_to_model_family(model_name: str) -> str:
    lowered = model_name.lower()
    for pattern, family in MODEL_NAME_FAMILY_PATTERNS:
        if pattern in lowered:
            return family
    return "Unknown"

# Token IDs that terminate generation early, per model family. Families with an
# empty list have no known terminators yet (TODO).
GENERATION_TERMINATOR_ID_DICT = {
    "gpt-2": [],
    "gpt-3": [],
    "bloomz": [],
    "opt": [],
    ...
    "tinyllama": [2, 13],
    "llama2": [2, 13],
    "llama3": [198, 627, 128001],
    "llama31": [198, 627, 128001],
    "llama32": [198, 627, 128001],
}
